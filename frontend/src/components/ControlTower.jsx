...
function ControlTower() {
  ...
  const fetchAIInsights = async () => {
    console.log('[CONTROL TOWER] 🤖 Fetching AI insights...')
    try {
      setLoadingInsights(true)
      const response = await tmsAPI.getOperationsIntelligence('today')
      console.log('[CONTROL TOWER] ✅ API Response:', response.data)
      
      if (response.data.success) {
        setAiInsights(response.data)
        console.log('[CONTROL TOWER] ✅ AI Insights loaded successfully')
        console.log('[CONTROL TOWER] Report preview:', response.data.report?.substring(0, 100))
      } else {
        console.error('[CONTROL TOWER] ❌ API returned success:false:', response.data)
      }
    } catch (error) {
      console.error('[CONTROL TOWER] ❌ Failed to fetch AI insights:', error)
      console.error('[CONTROL TOWER] Error details:', error.response?.data || error.message)
    } finally {
      setLoadingInsights(false)
      console.log('[CONTROL TOWER] 🏁 Fetch complete')
    }
  }

...
def operations_intelligence_dashboard():
    ...
    try:
        ...
        print("[OPERATIONS INTELLIGENCE] Initializing database client...")
        db = SupabaseClient()
...
Generated by 4 AI agents using LangChain + Google Gemini"""
        ...
        print(f"[OPERATIONS INTELLIGENCE] Error type: {type(e).__name__}")
        import traceback
        error_trace = traceback.format_exc()
        print(f"[OPERATIONS INTELLIGENCE] Full traceback:\n{error_trace}")
        
        # Fallback to demo data if database fails
        return jsonify({
            "success": True,
            "report": f"⚠️ Unable to fetch live data (Error: {type(e).__name__})\n\nDisplaying cached insights. Please refresh in a moment.\n\nTechnical details: {str(e)[:100]}",
